#!/usr/bin/env bash
# forgeward-diff-hash.sh <base-ref>
#
# Stable sha256 over the REVIEWED STATE of the branch vs <base-ref>. The marker
# stores this; the PreToolUse hook recomputes it at push time. Contract:
...
set -uo pipefail
base="${1:?usage: forgeward-diff-hash.sh <base-ref>}"
...
# Part 2 — canonical snapshot of root package.json at HEAD, version neutralized.
pkg_part=""
