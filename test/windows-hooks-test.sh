...
case "$LEGACY_PRETOOL_COMMAND" in
  *'%PLUGIN_ROOT%\scripts\forgeward-gate-check.cmd'*pretooluse*)
    ok "legacy Claude hook definition has a native-Windows fallback for stale Codex dispatch" ;;
  *) nok "legacy stale-trust Windows fallback" "$LEGACY_PRETOOL_COMMAND" ;;
esac
...
if [ "$no_git_prompt" = "0|" ] && [ "$no_git_pre" = "0|" ] && [ "$no_git_legacy_pre" = "0|" ]; then
  ok "Codex and stale-trust routes fail open silently instead of using WSL bash"
else
  nok "no-Git-Bash fail-open contract" "prompt=$no_git_prompt pretool=$no_git_pre legacy=$no_git_legacy_pre"
fi
