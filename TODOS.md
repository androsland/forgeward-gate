...
- **forgeward still describes itself as a gate *for gstack*, and three of five couplings
  ...
  **standalone-first, gstack named as an integration**: forgeward describes what it does on
  ...
  Two couplings from the list above are still live, and each gets its own PR under the
  executable-behaviour rule — `agents/security-reviewer.md:3,14`, and
  `supply-chain-reviewer`'s `/cso` probe. The `UserPromptExpansion` matcher gap is unchanged
